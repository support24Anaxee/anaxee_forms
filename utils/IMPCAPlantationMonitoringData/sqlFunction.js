...
const sendToDb = (data) => {
    return new Promise((resp, rej) => {
        cloudSql.query(
            `INSERT INTO impca_plantation_monitoring_real_time_data (FormId, data) VALUES (?, ?);`,
...
const updateAirtableIdInDb = (airtableId, submissionId) => {
    return new Promise((resp, rej) => {
        cloudSql.query(
            `UPDATE impca_plantation_monitoring_real_time_data SET airtableId = ? WHERE FormId = ?;`,
