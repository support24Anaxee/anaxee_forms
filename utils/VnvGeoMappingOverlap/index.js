...
const VnvGeoMappingOverlapFunction = async (formData) => {
    try {
        ...
        if(form.length > 0){
            ...
            if(typeof parsedData.Tree_Details === 'object' && parsedData.Tree_Details !== null && !Array.isArray(parsedData.Tree_Details)){
                const treeDetails = parsedData.Tree_Details;
                for(const key in treeDetails){
                    parsedData.Tree_Details[`1_${key}`] = treeDetails[key];
                    delete parsedData.Tree_Details[key];
                }
            }
...
module.exports = { VnvGeoMappingOverlapFunction };