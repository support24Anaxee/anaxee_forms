...
const sendToDb = (data) => {
    return new Promise((resp, rej) => {
        cloudSql.query(
            `INSERT INTO impca_sapling_monitoring_after_2_3_days_data (FormId, data) VALUES (?, ?);`,
...
const updateAirtableIdInDb = (airtableId, submissionId) => {
    return new Promise((resp, rej) => {
        cloudSql.query(
            `UPDATE impca_sapling_monitoring_after_2_3_days_data SET airtableId = ? WHERE FormId = ?;`,
