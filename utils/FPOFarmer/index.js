...
function renameAndDeleteKey(data, key, newName) {
    if (data[key]) {
...
const FPOFarmerFunction = async (formData) => {
    try {
        ...
        if(form.length > 0){
            ...
            if(parsedData.Crop_Details_count === 1){
                //add add 1_ in each key in Crop_Details object and remove Crop_Details each old key
                const cropDetails = parsedData.Crop_Details;
                for(const key in cropDetails){
                    parsedData.Crop_Details[`1_${key}`] = cropDetails[key];
                    delete parsedData.Crop_Details[key];
                }
            }

            const finalParsedData = flattenObject(parsedData);
            delete finalParsedData.Crop_Details_count;


            renameAndDeleteKey(finalParsedData, "Village_Name", "Village");
            renameAndDeleteKey(finalParsedData, "Tehsil_Name", "Tehsil");
            renameAndDeleteKey(finalParsedData, "District_Name", "District");
            renameAndDeleteKey(finalParsedData, "State_Name", "State");
            ...
            const contact_number = finalParsedData.Farmer_Mobile_Number;
            ...
            const runnerData = {
                ...
                projectName: "Gram Sathi Farmer Form",
            ...
            const insertDatainDb = await sendToDb(airtableData, contact_number);
            ...
            const insertDataInAirtable = await insertInAirtable(airtableData, insertDatainDb[0]);
            const updateAirtableId = await updateAirtableIdInDb(insertDataInAirtable[0].id, submissionId, insertDatainDb[0]);
...
module.exports = { FPOFarmerFunction };