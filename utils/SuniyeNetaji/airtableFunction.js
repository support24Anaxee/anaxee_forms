...
const suniye_netaji = airtable.base("app8fC1r2d9tVPXkp");
const table = suniye_netaji("tblNRE59LxIyEKaY0");
const table1 = suniye_netaji("tbljEcDrtOK3UyRdR");
...
const insertInAirtable = async (data, is_duplicate) => {
    try{
        let finaltable;
        if(is_duplicate) finaltable = table1;
        else finaltable = table;
        const insertResponse = await finaltable.create(
            [
                {
                    fields: data,
                },
