...
const sendToDb = (data, contact_number) => {
    return new Promise((resp, rej) => {
        cloudSql.query(
            `INSERT INTO suniye_netaji_data (FormId, data, contact_number) VALUES (?, ?, ?);`,
            ...
            (err, result) => {
                if (err) {
                    if (err && err.code === "ER_DUP_ENTRY") {
                        ...
                        cloudSql.query(
                            `INSERT INTO suniye_netaji_duplicate_data (FormId, data, contact_number) VALUES (?, ?, ?);`,
                            [data.FormId, JSON.stringify(data), contact_number],
                            (err, result) => {
                                if (err) {
                                    ...
                                    let msgBody = {
                                        ...
                                        sms: [{
                                            message: `Mob-No. ${contact_number} | Duplicate Number Found in Suniye Netaji - Aaj Tak Form | Register new Customer | Ask Customer before Registration | Duplicate data will be rejected. Thank you. Anaxee`,
                                            to: [fenumber],
                                        }],
                                    };
...
const updateAirtableIdInDb = (airtableId, submissionId, is_duplicate) => {
    return new Promise((resp, rej) => {
        if(is_duplicate){
            cloudSql.query(
                `UPDATE suniye_netaji_duplicate_data SET airtableId = ? WHERE FormId = ?;`,
            ...
            cloudSql.query(
                `UPDATE suniye_netaji_data SET airtableId = ? WHERE FormId = ?;`,
                [airtableId, submissionId],
                (err, result) => {
                    if (err) {
                        console.log("DB_SQL_UPDATE_FAILED", new Error(err));
