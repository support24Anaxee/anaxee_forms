...
const sendToDb = (data, contact_number) => {
    return new Promise((resp, rej) => {
        cloudSql.query(
            `INSERT INTO kshema_farmer_data (FormId, data, contact_number) VALUES (?, ?, ?);`,
            ...
            (err, result) => {
                if (err) {
                    if (err && err.code === "ER_DUP_ENTRY") {
                        ...
                        cloudSql.query(
                            `INSERT INTO kshema_farmer_duplicate_data (FormId, data, contact_number) VALUES (?, ?, ?);`,
                            [data.FormId, JSON.stringify(data), contact_number],
                            (err, result) => {
                                if (err) {
...
const updateAirtableIdInDb = (airtableId, submissionId, is_duplicate) => {
    return new Promise((resp, rej) => {
        if(is_duplicate){
            cloudSql.query(
                `UPDATE kshema_farmer_duplicate_data SET airtableId = ? WHERE FormId = ?;`,
            ...
            cloudSql.query(
                `UPDATE kshema_farmer_data SET airtableId = ? WHERE FormId = ?;`,
                [airtableId, submissionId],
                (err, result) => {
                    if (err) {
                        console.log("DB_SQL_UPDATE_FAILED", new Error(err));
