...
const SchaefflerVillageCommitteeFunction = async (formData) => {
    try {
        ...
        if(form.length > 0){
            ...
            if(typeof parsedData.Committee_Member_Details === 'object' && parsedData.Committee_Member_Details !== null && !Array.isArray(parsedData.Committee_Member_Details)){
                const CommitteeDetails = parsedData.Committee_Member_Details;
                for(const key in CommitteeDetails){
                    parsedData.Committee_Member_Details[`1_${key}`] = CommitteeDetails[key];
                    delete parsedData.Committee_Member_Details[key];
                }
            }
            if(typeof parsedData.Photos_Members === 'object' && parsedData.Photos_Members !== null && !Array.isArray(parsedData.Photos_Members)){
                const Photos_Members = parsedData.Photos_Members;
                for(const key in Photos_Members){
                    parsedData.Photos_Members[`1_${key}`] = Photos_Members[key];
                    delete parsedData.Photos_Members[key];
                }
            }            
...
module.exports = { SchaefflerVillageCommitteeFunction };