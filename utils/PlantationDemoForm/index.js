    ...
    try {
        ...
        if(form.length > 0){
            ...
            for(const key in finalParsedData){
                ...
                else if(formFields[key.replace(/^\d+_/, '')] === "binary"){
                    const url = await uploadFileToBlob(attachmentUrl + finalParsedData[key], finalParsedData[key]);
                    if(airtableData[key.replace(/^\d+_/, '').replace(/_/g, ' ')])  airtableData[key.replace(/^\d+_/, '').replace(/_/g, ' ')].push({url : url})
                    
                    else airtableData[key.replace(/_/g, ' ')] = [{ url : url }]
            ...
            if(finalParsedData["Mapping_Of_Farm"]) airtableData["count of tree"] = await countCoordinates(finalParsedData["Mapping_Of_Farm"]);
